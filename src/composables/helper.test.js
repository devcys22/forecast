...
import { getImage, dayToker } from './helper.js';
...
expect(dayToker.length).toBe(7); // dayToKor 배열이 7개인지 확인
dayToker.forEach((day) => {
    expect(typeof day).toBe('string'); // dayToKor 배열에 문자열이 아닌 다른 형태의 값이 있는지 확인
});
expect(dayToker).toEqual(expected); // dayToKor 배열이 올바른 순서로 있는지 확인
});
});
